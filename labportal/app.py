...
CLUSTER_IP_RANGES = {
    ...
    "sno1": ("192.168.200.10", "192.168.200.10"),
...
def api_status():
    ...
    console_urls = {
        name: config.console_url(name, config.base_domain(), public_host)
        for name in clusters
    }
    with get_db_ctx() as conn:
...
def activate_console():
    ...
    console_url = config.console_url(
        cluster_name, config.base_domain(), public_host
    )
    if not console_url:
        return jsonify(error="Console forwarding is not configured for this cluster"), 404
